...
Description: A Plugin For WordPress CRUD  Using Ajax & WP List Table
...
function my_menu_pages(){
    ...
    add_submenu_page('new-entry', 'CRUD Application', 'New Data', 'manage_options', 'new-entry', 'my_menu_output' );
    add_submenu_page('new-entry', 'CRUD Application', 'View Data', 'manage_options', 'view-entries', 'my_submenu_output' );
}
...
class EntryListTable extends WP_List_Table {
    ...
    function prepare_items() {
      ...
      $order = (isset($_REQUEST['order']) && in_array($_REQUEST['order'], array('asc', 'desc'))) ? $_REQUEST['order'] : 'desc';
        ...
        $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE `name` LIKE '%".$_REQUEST['s']."%' OR `address` LIKE '%".$_REQUEST['s']."%' ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged * $per_page), ARRAY_A);
		  } else {
